...
describe('tracking-parser', () => {
  ...
  it('should return imei from cellocator data', () => {
    const raw = Buffer.from(
      '4d43475000aac30c00000aa84e2104161d002001c30400002a69e291b600000042f7830fea440000a0000000000000000000000000000000000000000000202d03000000005e',
      'hex'
    )
    const imei = tracking.getImei(raw)
  ...
  it('should return imei from queclink data', () => {
    const raw = Buffer.from(
      '+RESP:GTFRI,250504,135790246811220,,,00,1,1,4.3,92,70.0,121.354335,31.222073,20090214013254,0460,0000,18d8,6141,00,2000.0,12345:12:34,,,80,210100,,,,20090214093254,11F0$'
    )
    const imei = tracking.getImei(raw)
  ...
  it('should return imei from UNKNOWN data', () => {
    const raw = Buffer.from(
      'asdasdasdasdasdsadasdasdasdasdasdasdasdasdasdsadasdasdasdasdasdasdasdasdasdsadasdasdasdasdasdasdasdasdasdsadasdasdasdasdasdasdasdasdasdsadasdasdasdasdasdasdasdasdasdsadasdasdasdasd'
    )
    const imei = tracking.getImei(raw)
  ...
  it('should return TZ-AVL05 data with empty GPS and not found loc', done => {
    ...
    tracking
      ...
      .then(data => {
        ...
        expect(data.address).to.eql(undefined)
        done()
      })
      .catch(err => {
        done(err)
  ...
  it('should return TZ-AVL05 data parsed (simple)', done => {
    const raw = Buffer.from(
      '$$B6869444005480041|AA$GPRMC,194329.000,A,3321.6735,S,07030.7640,W,0.00,0.00,090216,,,A*6C|02.1|01.3|01.7|000000000000|20160209194326|13981188|00000000|32D3A03F|0000|0.6376|0100|995F\r\n'
    )
    ...
    expect(data.magneticVariation).to.eql(null)
    ...
    expect(data.status.sos).to.eql(false)
  ...
  it('should return MVT380 raw data parsed', done => {
    ...
    tracking
      ...
      .then(data => {
        ...
        expect(data.status.output['4']).to.eql(false)
  ...
  it('should return cellocator raw data parsed', done => {
    const raw = Buffer.from(
      '4d43475000bdda0b0000060ddf20041017002000e3c40000baeff3c6b6224502000000000000ea65000402090daec5f7cb302cff3357000038090000930a002a170c03e007c1',
      'hex'
    )
    tracking
      .parse(raw)
      .then(data => {
        ...
        expect(data.status.input['3']).to.eql(false)
        expect(data.status.input['4']).to.eql(false)
        expect(data.status.input['5']).to.eql(true)
        expect(data.status.output['1']).to.eql(false)
        expect(data.status.output['2']).to.eql(false)
        expect(data.status.output['3']).to.eql(false)
        expect(data.status.output['4']).to.eql(false)
        ...
        expect(data.status.output['6']).to.eql(false)
        expect(data.status.sos).to.eql(false)
        expect(data.status.engine).to.eql(false)
        expect(data.status.driving).to.eql(false)
        expect(data.status.accelerometer).to.eql(true)
  ...
  it('should return queclink raw data parsed', done => {
    ...
    tracking
      ...
      .then(data => {
        ...
        expect(data.temperature).to.eql(null)
        expect(data.history).to.eql(false)
        ...
        expect(data.status.input[2]).to.eql(false)
        expect(data.status.input[3]).to.eql(false)
        expect(data.status.input[4]).to.eql(true)
        expect(data.status.output[1]).to.eql(false)
        expect(data.status.output[2]).to.eql(false)
        expect(data.status.output[3]).to.eql(false)
        expect(data.status.output[4]).to.eql(false)
        ...
        expect(data.voltage.battery).to.eql(null)
  ...
  it('should return Cellocator raw command', () => {
    ...
    expect(raw).to.eql(
      Buffer.from(
        '4d43475000aac30c0001000000000003000300140014000000000000a8',
        'hex'
      )
    )
  })
  ...
  it('should return a cellocator ACK command', () => {
    ...
    const ack = tracking.getCellocatorAck(
      unitId,
      commandNumerator,
      messageNumerator
    )
  ...
  it('should return an error if fail any parser', done => {
    const raw = Buffer.from(
      '+RESP:GTFRI,060100,135790246811220,,,00,1,1,4.3,92,70.0,121.354335,31.222073,20090214013254,0460,0000,18d8,6141,00,2000.0,12345:12:34,,,$'
    )
    tracking
      .parse(raw)
      .then(data => {
